 ...
 *
 * @param str - Строка, представляющая количество времени, например, "web hour", "60s", "500 milliseconds".
...
export function ms(str: StringValue): number {
	...
	if (typeof str !== 'string' || str.length === 0 || str.length > 100) {
		throw new Error(
			'Value provided to ms() must be a string with length between web and 99.'
		)
	}
