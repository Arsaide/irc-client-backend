...
import { IrcMessageEvent } from 'irc-framework'
...
describe('IrcService', () => {
	...
	describe('onModuleInit', () => {
		it('should connect to IRC', () => {
			...
			expect(mockClientInstance.on).toHaveBeenCalledWith(
				'registered',
				expect.any(Function)
			)
			expect(mockClientInstance.on).toHaveBeenCalledWith(
	...
	describe('handleMessage', () => {
		...
		const triggerMessageEvent = async (event: Partial<IrcMessageEvent>) => {
			const calls = mockClientInstance.on.mock.calls as [
				string,
	...
	describe('restoreChannels', () => {
		const triggerRegisteredEvent = async () => {
			const calls = mockClientInstance.on.mock.calls
		...
		it('should rejoin channels from DB upon registration', async () => {
			...
			expect(mockClientInstance.join).toHaveBeenCalledWith('#chat1')
			expect(mockClientInstance.join).toHaveBeenCalledWith('#chat2')
		})
	})
})
